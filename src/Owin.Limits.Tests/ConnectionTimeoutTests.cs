namespace Owin.Limits
{
    ...
    public class ConnectionTimeoutTests
    {
        ...
        public async Task When_time_out_is_not_triggered_then_should_throw()
        {
            ...
            HttpClient httpClient = CreateTestServer(getConnectionTimeout).CreateHttpClient();
        ...
        public async Task When_time_out_is_not_triggered_then_get_OK()
        {
            ...
            stream.Write(new byte[2048], 0, 2048);
            stream.Position = 0;
            var response = await httpClient.PostAsync("http://example.com", new StreamContent(stream));
            response.StatusCode.Should().Be(HttpStatusCode.OK);
        }

        private static OwinTestServer CreateTestServer(Func<TimeSpan> getConnectionTimeout)
        {
            return OwinTestServer.Create(builder =>
            {
                ...
                builder
                    ...
                    .Use(async context =>
                    {
                        ...
                        var i = await context.Request.Body.ReadAsync(buffer, 0, buffer.Length);
